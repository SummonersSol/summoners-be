...
export default class WildMonster extends Base {
    ...
    applyStats = async() => {
        //boss or wild
        //get metadata
        let metadata = await DB.executeQueryForSingleResult<MonsterBaseMetadata>(`
            SELECT 
                mb.*,
        ...
        let allSkillsRes = await DB.executeQueryForResults<MonsterEquippedSkill>(allSkillsQuery);
