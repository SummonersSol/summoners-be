...
import { fillableColumns, UserCompletedLesson } from "../Models/UserCompletedLesson";
const table = 'user_completed_lessons';

// init entry for UserCompletedLesson
...
export const view = async(id: number) => {
    ...
    const result = await DB.executeQueryForSingleResult<UserCompletedLesson>(query);
...
export const find = async(whereParams: {[key: string]: any}) => {
    ...
    const result = await DB.executeQueryForResults<UserCompletedLesson>(query);
