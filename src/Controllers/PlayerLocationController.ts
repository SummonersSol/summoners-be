...
import { fillableColumns, PlayerLocation } from "../Models/PlayerLocation";
const table = 'player_locations';

// init entry for PlayerLocation
...
export const view = async(id: number) => {
    ...
    const result = await DB.executeQueryForSingleResult<PlayerLocation>(query);
    return result;
}
...
export const find = async(whereParams: {[key: string]: any}) => {
    ...
    const results = await DB.executeQueryForResults<PlayerLocation>(query);
    return results ?? [];
}
